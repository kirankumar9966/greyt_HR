...
import '../controllers/regularisation_pending_details_page_controller.dart';

class RegularisationPendingDetailsPageView extends StatefulWidget {
  final int id;
  final String token;

  const RegularisationPendingDetailsPageView({
    super.key,
    required this.id,
    required this.token,
  });
  ...
  @override
  State<RegularisationPendingDetailsPageView> createState() => _RegularisationPendingDetailsPageView();
}

class _RegularisationPendingDetailsPageView extends State<RegularisationPendingDetailsPageView> {
  late RegularisationPendingDetailsPageController controller;

  @override
  void initState() {
    super.initState();
    controller = Get.put(RegularisationPendingDetailsPageController());
                    ...
                    children: [
                      ...
                      Center(
                        child: OutlinedButton(
                          onPressed: () =>  _showTimelineDialog(context,Status,displayName,EmployeeName,statusDateString,submittedDateString),
...
void _showTimelineDialog(BuildContext context, int status,String? displayName,String? EmployeeName,String? statusDateString,String? submittedDateString) {
        ...
        content: Column(
          ...
          children: [
            ...
            _buildTimelineRow(
              ...
              dateTime: (status == 2 || status == 3 || status == 4) ?  formattedStatusDate : '',
            ),
            if (status == 5)
              const SizedBox(height: 8),
            if (status == 5)
            ...
            _buildTimelineRow(
              ...
              label: 'Submitted by $EmployeeName',
